...
import { getVeterinarians } from "../veterinarians/VeterinarianService";
import VetEditableRows from "../veterinarians/VetEditableRows";
...
const VeterinarianComponent = () => {
  ...
  const fetchVeterinarians = () => {
    getVeterinarians()
      .then((data) => {
        console.log("vet Data : ", data.data);
  ...
  const handleToggleAccountLock = async (vet) => {
    try {
      ...
      // Optimistically update the UI to reflect the new 'active' state
      setVeterinarians(
        veterinarians.map((theVet) =>
          theVet.id === vet.id
            ? { ...theVet, active: !theVet.active }
            : theVet
        )
      );
      setSuccessMessage(response.message);
              ...
              <tr key={vet.id}>
                ...
                <td>
                  <OverlayTrigger
                    overlay={
                      <Tooltip id={`tooltip-view-${index}`}>
                        {vet.active ? "Lock" : "Unlock"} vet account
                      </Tooltip>
                    }>
                    <span
                      onClick={() => handleToggleAccountLock(vet)}
                      style={{ cursor: "pointer" }}>
                      {vet.active ? <BsUnlockFill /> : <BsLockFill />}
                    </span>
                  </OverlayTrigger>
                </td>
